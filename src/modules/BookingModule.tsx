    ...
    <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-card rounded-2xl shadow-2xl max-w-md w-full p-6 text-center animate-bounce-in" onClick={e => e.stopPropagation()}>
        ...
        <p className="text-sm text-muted-foreground mb-6">
          Customer still has remaining balance. Please check <strong>Booking Management</strong> to settle.
        </p>
...
export default function BookingModule() {
  ...
  const total = baseFee + roomAddOn + tableAddOn + tourAddOn + corkage;
  ...
  const handleSave = useCallback(async () => {
    ...
    setSaving(false);
  }, [customerName, bookingType, checkIn, checkOut, tourType, corkage, a, c, headcount, total, deposit, balance, paymentStatus, payment, addOnRoom, numTables]);
    ...
    <>
      ...
      <ModuleShell title="Booking" icon={<CalendarDays size={20} />} onSave={handleSave} saveLabel="Record Booking" saving={saving}>
        ...
        <div>
          ...
          {tourType !== "None" && (
            <p className="text-xs text-muted-foreground mt-1">Rate: ₱{tourAddOn.toLocaleString()}</p>
          )}
        </div>
        ...
        <div>
          ...
          {addOnRoom !== "None" && (
            <p className="text-xs text-muted-foreground mt-1">Rate: ₱{(addOnRoom === "Kubo Room" ? kuboRate : barkadaRate).toLocaleString()}</p>
          )}
        </div>
        ...
        <div>
          ...
          {numTables > 0 && (
            <p className="text-xs text-muted-foreground mt-1">{numTables} × ₱{tableRate.toLocaleString()} = ₱{tableAddOn.toLocaleString()}</p>
          )}
        </div>
        ...
        <div className="pos-card border-primary/30">
          ...
          {(roomAddOn > 0 || tableAddOn > 0 || tourAddOn > 0 || corkage > 0) && (
            <div className="text-xs text-muted-foreground mt-2 space-y-0.5">
              <p>Base: ₱{baseFee.toLocaleString()}</p>
