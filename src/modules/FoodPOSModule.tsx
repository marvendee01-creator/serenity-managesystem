...
function POSScreen({ inventory, onSold }: { inventory: FoodInventoryItem[]; onSold: () => void }) {
  ...
  const handleSave = useCallback(async () => {
    ...
    if (cash < total) { toast.error("Insufficient cash"); return; }
    ...
    try {
      ...
      toast.success(`Sale recorded — Change ${formatPeso(change)}`);
  ...
  return (
    <ModuleShell title="Food POS" icon={<UtensilsCrossed size={20} />} onSave={handleSave} saveLabel="Record Sale" saving={saving}>
      <div>
        ...
        <input type="text" className="pos-input w-full" value={customerName} onChange={(e) => setCustomerName(e.target.value)} placeholder="Walk-in" />
      </div>
